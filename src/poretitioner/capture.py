...
from .hdf5 import HDF5_DatasetSerializable
...
class CaptureFile(BaseFile):
    ...
    def initialize_from_bulk(
        ...
        SEGMENT_METADATA = SegmentationMeta(segmenter_name, version, filters, segment_config.terminal_capture_only, segment_config.open_channel_prior_mean, segment_config.open_channel_prior_stdv, good_channels, capture_windows_group)
    ...
    def write_capture(self, raw_signal: RawSignal, metadata: CaptureMetadata, log: Logger = None):
        ...
        channel_id = Channel(metadata.channel_number, metadata.calibration, metadata.open_channel_pA, metadata.sampling_rate)
        #signal = HDF5_Signal.__new__(HDF5_Signal, raw_signal)
        signal = HDF5_Signal(raw_signal, metadata.start_time_bulk, metadata.start_time_local, metadata.duration, metadata.voltage_threshold, metadata.open_channel_pA, read_id, metadata.ejected, metadata.channel_number)
