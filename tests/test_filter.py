...
def check_capture_ejection_by_read_test():
    ...
    with h5py.File(f5_fail, "r") as f5:
        with pytest.raises(ValueError) as e:
            filter.check_capture_ejection_by_read(f5, bad_read_id)
...
def check_capture_ejection_test():
    ...
    tol_obs = 100
    assert filter.check_capture_ejection(end_capture, voltage_ends, tol_obs=tol_obs)

    end_capture = 1200
    voltage_ends = [0, 1000, 2000, 3000]
    tol_obs = 100
    assert not filter.check_capture_ejection(end_capture, voltage_ends, tol_obs=tol_obs)
    ...
    with h5py.File(filter_f5_fname, "r") as f5:
        for g in f5.get("/"):
            if "read" in g:
                ...
                passed = filter.apply_filters_to_read(config, f5, read_id, filter_name)
                if passed:
...
def filter_and_store_result_test():
    ...
    for filter_name in filter_names:
        filter.filter_and_store_result(config, [filter_f5_fname], filter_name, overwrite=True)
...
def write_filter_results_test():
    ...
    with h5py.File(filter_f5_fname, "a") as f5:
        filter.write_filter_results(f5, config, read_ids, filter_name)
